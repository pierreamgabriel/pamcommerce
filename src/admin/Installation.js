...
const db = useSelector((state) => state.installDetails);	
...
const install = async () => {
await axios.post('./api/create-db-table.php', {
...
<label className="form-label">Choose a language:</label>		   
 <select defaultValue={site.lang} className="form-select" onChange={e=> dispatch(setSettings(
 {key: "lang", value: e.target.value}))}>
  <option>Choose a language</option>
  {langOptions.lang.map((option, index) => {
  return(
  <option key={index} value={option}>{option}</option>
	...
	<button type="button" className="btn btn-secondary" onClick={() => setState("create")}>
...
<div className="container-fluid install-form-main">		
	<form>		
  <div className="mb-3">
    <label className="form-label install-label">{langFile._createDB[0]}</label>
    <input type="text" name="dbname" value={db.dbname} onChange={handleChange} 
	className="form-control" required/>
	<p className="install-db-text">{langFile._createDB[1]}</p>	
  </div>			
  <div className="mb-3">
    <label className="form-label install-label">{langFile._createDB[2]}</label>
    <input type="text" name="hostuser" onChange={handleChange} className="form-control" required/>
	<p className="install-db-text">{langFile._createDB[3]}</p>	
  </div>
  <div className="mb-3">
    <label className="form-label install-label">{langFile._createDB[4]}</label>
    <input type="password" name="hostpass" onChange={handleChange} className="form-control" />
	<p className="install-db-text">{langFile._createDB[5]}</p>	
  </div>
  <div className="mb-3">
    <label className="form-label install-label">{langFile._createDB[6]}</label>
    <input type="text" name="hostname" value={db.hostname} onChange={handleChange} 
	className="form-control" required/>
	<p className="install-db-text">{langFile._createDB[7]}</p>	
  </div>		
  <button type="button" className="btn btn-secondary" onClick={connect}>
  {langFile._createDB[8]}</button>
...
<h5 className="install-finish-title">{langFile._install[0]}</h5>		
	<form>		
  <div className="mb-3">
    <label className="form-label install-label">{langFile._install[1]}</label>
    <input type="text" name="sitename" onChange={handleChange} className="form-control" required/>
  </div>			
  <div className="mb-3">
    <label className="form-label install-label">{langFile._install[2]}</label>
    <input type="email" name="email" onChange={handleChange} className="form-control" required/>
  </div>
  <div className="mb-3">
    <label className="form-label install-label">{langFile._install[3]}</label>
    <input type="password" name="password" onChange={handleChange} className="form-control" required/>
  </div>
  <button type="button" onClick={install} className="btn btn-secondary">{langFile._install[4]}</button>
...
<div className="row">		   
<img src={logo} className="img-fluid install-logo-img mx-auto" />	
</div>		   
</div>		
	);
}
